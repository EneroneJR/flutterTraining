...
class MovementListItem {
  ...
  const MovementListItem({required this.movement, required this.categoryName});
}
...
class TransactionsRepository {
  ...
  Stream<List<MovementListItem>> watchAllMovements() {
    final query = database.select(database.expenseMovements).join([
      leftOuterJoin(
        database.categories,
        database.categories.id.equalsExp(database.expenseMovements.categoryId),
      ),
    ]);
    ...
    return query.watch().map((rows) {
      return rows.map((row) {
        final movement = row.readTable(database.expenseMovements);
  ...
  Future<int> addMovement({
    ...
    return database
        .into(database.expenseMovements)
        .insert(
          ExpenseMovementsCompanion.insert(
            ...
            paymentMethod: Value(_normalizeOptionalText(paymentMethod)),
            notes: Value(_normalizeOptionalText(notes)),
          ),
        );
  }
  ...
  Future<int> updateMovement({
    ...
    final query = database.update(database.expenseMovements)
  ...
  Future<int> deleteMovement(int id) {
    final query = database.delete(database.expenseMovements)
      ..where((row) => row.id.equals(id));
